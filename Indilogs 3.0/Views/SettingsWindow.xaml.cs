...
namespace IndiLogs_3._0.Views
{
    public partial class SettingsWindow : Window
    {
        ...
        private void EditDefaultFilter_Click(object sender, RoutedEventArgs e)
        {
            ...
            // Load current default filter (user's or factory) — binary APP gets its own factory defaults
            var factoryFilter = vm.HasBinaryAppLogs
                ? DefaultConfigurationService.GetFactoryBinaryAppPlcFilter()
                : DefaultConfigurationService.GetFactoryPlcFilter();
            var currentFilter = vm.FilterVM.DefaultPlcFilter ?? factoryFilter;
        ...
        private void EditDefaultColoring_Click(object sender, RoutedEventArgs e)
        {
            ...
            // Load current default coloring rules (user's or factory) — binary APP gets its own factory defaults
            var currentRules = defaults?.MainDefaultColoringRules;
            if (currentRules == null || currentRules.Count == 0)
            {
                currentRules = vm.HasBinaryAppLogs
                    ? DefaultConfigurationService.GetFactoryBinaryAppColoringRules()
                    : DefaultConfigurationService.GetFactoryMainColoringRules();
            }

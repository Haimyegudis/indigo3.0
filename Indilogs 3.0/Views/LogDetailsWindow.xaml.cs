...
namespace IndiLogs_3._0.Views
{
    public partial class LogDetailsWindow : Window
    {
        ...
        public LogDetailsWindow(LogEntry log)
        {
            ...
            {
                // Header info
                TimeText.Text = log.Date.ToString("yyyy-MM-dd HH:mm:ss.ffffff");
                ...
                // Add ExtraFields as dynamic tabs (for plugin/Different Logs entries)
                if (log.ExtraFields != null && log.ExtraFields.Count > 0)
                {
                    foreach (var kvp in log.ExtraFields)
                    {
                        var textBox = new TextBox
                        {
                            Text = kvp.Value ?? "(empty)",
        ...
        private void CopyAll_Click(object? sender, RoutedEventArgs e)
        {
            ...
            var sb = new StringBuilder();
            sb.AppendLine($"Timestamp: {_log.Date:yyyy-MM-dd HH:mm:ss.ffffff}");
            ...
            if (_log.ExtraFields != null && _log.ExtraFields.Count > 0)
            {
                foreach (var kvp in _log.ExtraFields)
                {
                    sb.AppendLine();
